...
def install_sequence(cfg: Config) -> list[Step]:
    ...
    **顺序不是随便排的**，三条硬理由：
    ...
    return [
        Step(0, "把水表的蓝色盖板**整个拆掉**（保留底环和铰链销）", "-",
             present=("meter",), check_path=False, tool="徒手",
             note="盖板是卡在铰链销上的，掰下来即可；**铅封在表体上，不在盖板上**，"
                  "所以拆它不动铅封。拆完表盘上只剩蓝色底环和那根铰链销。"
                  "为什么非拆不可：光学读表本来就要求盖板常开，而翻开的盖板会立成"
                  "一堵墙挡住表盘墙侧外缘（可见半径只剩 24.9，指针小盘最外圈差 0.12mm）。"
                  "拆掉之后可见半径回到 27.4，余量 +2.4mm，镜片也能从 54 加长到 58。"
                  "★ 拆下来的盖板**按表号装袋留好**（上面有二维码和检定证标签），"
                  "水司要查的时候能装回去。",
             pokayoke="拆掉之后表面只剩一根铰链销，装配时不再有『盖板翻到位没有』"
                      "这一类需要人判断的前提条件 —— **能把一个人为判断从流程里"
                      "删掉，比给它加一道检查更彻底。**"),
        ...
        Step(11, "确认表盘上只剩底环和铰链销（盖板第 0 步已拆）", "-",
             present=("meter", "body", "board", "slide_cover"), check_path=False,
             tool="徒手",
             note=f"装托板之前扫一眼：盖板拆了没有、铰链销有没有歪。"
                  f"★ 可见半径 {abs(dial_visible_y_min(cfg)):.1f}mm（表盘 "
                  f"{cfg.meter.dial_r:.1f}mm），墙侧最外那 "
                  f"{cfg.meter.dial_r - abs(dial_visible_y_min(cfg)):.1f}mm "
                  f"仍然看不到 —— 那是铰链销顶住镜片后缘的结果，"
                  f"里面没有要读的东西（四个指针小盘最外到 r=25，余量 2.4mm）。",
             pokayoke="盖板没拆的话，托板落下去会被它顶住、明显放不平 —— "
                      "被动防呆，但这一版已经不依赖它了。"),
