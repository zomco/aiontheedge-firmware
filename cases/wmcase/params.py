...
class Meter:
    ...
    cover_hinge_above_ring: float = 5.0   # [实测] 铰链轴高出固定环顶面（照片估读）
    cover_hinge_tol: float = 1.0     # [设计] 上面那个数的不确定度，**按最坏情况建模**
    #   ★ 铰链高一点，翻开的盖板就整片往房间侧多挪一点。所有让位量都按
    #     ``cover_hinge_above_ring + cover_hinge_tol`` 算（layout.cover_max_y），
    #     所以量偏了也不会装不进去 —— 代价是表盘外缘少看到约 0.8mm 半径。
    #   ★ **这是目前唯一还值得去量的尺寸**（上一轮扫描证明 cover_r / cover_t 不值得）：
    #     把 tol 从 1.0 收到 0.2，可视半径立刻多回来 0.6mm，直接变成指针盘的余量。
    #     量法：深度尺从固定环顶面量到铰链销中心。
    ...
    pointer_r: float = 5.5           # [估算] 单个指针小盘的半径
    #  ★ 以前只把指针盘当成一个**点**。现在墙侧外缘有一圈被蓝盖挡住看不见，
    #    判据必须用小盘的**整圈**去比 —— 挡住的是盘的下边缘，不是盘心。
    #    最靠墙的那两个盘，下边缘在 y = −18 − 5.5 = −23.5（自检 OPT-11）。
...
class Optics:
    ...
    # ---- 反射镜（外购件）----
    mirror_l: float = 53.0       # [采购] 前表面镜 长（沿 45° 斜面方向）
...
class Case:
    ...
    collar_seat_ir: float = 31.5     # [设计] 座圈内半径
    #   ★ 两条硬约束（自检 DIM-12）：
    #     · 必须 > 蓝色固定环外半径 30.9，否则座圈压在塑料环上而不是银圈上；
    #     · 径向厚度 (bezel_od/2 − 它) 不能超过银圈的径向厚度 10.2，
    #       否则沿会悬空伸进表盘上方，既没支承又挡光。
    #       当前 41.1 − 31.5 = 9.6 ≤ 10.2 ✓
