...
class Optics:
    ...
    # ---- 反射镜（外购件）----
    mirror_l: float = 54.0       # [采购] 前表面镜 长（沿 45° 斜面方向）
...
class Case:
    ...
    collar_seat_gap_deg: float = 55.0  # [设计] 墙侧让位口的**半角**
    #   ★ 座圈在墙侧必须开一个口，让位给两样东西：
    #     · 蓝盖的**铰链凸耳**（就长在 y≈−28 处，半径未知，宁可让开）；
    #     · 翻开的蓝盖本体 —— 有了这个口，装配时蓝盖开着也能把整机套下去，
    #       不必依赖"先合上盖板"这一步（那一步仍然写在说明书里，但不再是硬前提）。
    #     让位口不影响功能：整机重心在 +Y（吊舱侧），座圈受力的是 +Y 那半圈。
    ...
    # ---- 托板的两处"被别人决定"的边界 ----
    holder_cover_clear: float = 0.8  # [设计] 托板后端 ↔ 停放蓝盖 的间隙
    #   ★ 托板后端不再是垂直于 45° 面的方角，而是一刀**竖直**切下去
    #     （layout.holder_rear_cut_y）。方角的后上顶点会朝墙侧多探出
    #     (2·wall + mirror_t)/√2 ≈ 5mm —— 正好撞进翻开的蓝盖。
    #     竖直切面让托板贴着蓝盖那面平行地停住，同样的让位量下镜片能多伸 5mm，
    #     换回来的正是表盘外缘那 5mm 可见半径。
    holder_rear_wall: float = 1.0    # [设计] 竖直切面到镜片后缘的端壁厚度
    #   ↑ 镜片是从**前端**滑入的，后端这堵墙只当止挡，1.2mm（3 圈走线）够用。
