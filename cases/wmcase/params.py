...
class Meter:
    ...
    ring_h: float = 5.0              # [实测] 蓝色固定环高出表盘玻璃面（"大概 5mm"）
    ...
    #  ★ 盖板的半径和厚度**几乎不影响任何判定**，别浪费时间去量。
    #    做过灵敏度扫描（镜片 62mm 固定）：
    #        cover_r 从 30.9 扫到 36.0  → 最小可行停放角恒为 90°
    #        cover_t 从 2.0  扫到 5.0   → 最小可行停放角恒为 90°
    #    原因：卡住托板的是靠近**铰链**那一端，而铰链位置只由 ring_od 决定
    #    （y = −ring_od/2 = −30.9，已卡尺实测）。盖板的远端在半空中，够不着托板。
    #    真正要紧的两个数是 **ring_od**（已实测）和 **cover_open_deg**（已实测）。
    cover_r: float = 31.5            # [估算] 盖板半径（不敏感，见上）
    cover_t: float = 3.0             # [估算] 盖板厚度（不敏感，见上）
    cover_open_deg: float = 100.0    # [实测] 停放时盖板与表盘面的夹角（= 最大打开角）
    #  ★ 这个数**很敏感**，是整条链上唯一还带不确定性的输入。
