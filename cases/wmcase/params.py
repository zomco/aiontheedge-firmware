...
class Meter:
    ...
    cover_park_y: tuple[float, float] = (-46.0, -33.0)   # [估算] 蓝盖翻开后停放占据的 Y 带
    #   ★ 这两条都是照片目测，**没有实测**。它们决定"蓝盖翻开后会不会撞镜片托板"，
    #     而镜片托板为了接住表盘外缘的光，必须一直伸到 y≈−31。两者非常接近。
    #     自检 FIT-14 会算出实际重叠量，但结论只和这两个估算值一样可信 → 只报 WARN。
    #     验证方法：把蓝盖翻到平时停放的位置，量最外缘的 Y 和最高点 Z。
...
class Optics:
    ...
    # ---- 反射镜（外购件）----
    mirror_l: float = 70.0       # [采购] 前表面镜 长（沿 45° 斜面方向）
...
class Lighting:
    ...
    need_half_angle_deg: float = 21.0  # [设计] 覆盖整个表盘所需的最小半角
...
class Fasteners:
    ...
    m4_nut_af: float = 7.00      # [设计] M4 螺母对边
...
class Case:
    ...
    pad_inner_x: float = 32.0        # [设计] 托台内缘 X（越小越伸进光锥，当前余量充足）
    ...
    holder_bracket_x: tuple[float, float] = (32.0, 38.0)
    ...
    board_rib_len: float = 4.0       # [设计] 导轨沿 Y 的长度
